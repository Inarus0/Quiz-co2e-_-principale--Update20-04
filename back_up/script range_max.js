...
let timer;
let rangeInput = document.getElementById('yourRangeInputId'); // Replace 'yourRangeInputId' with the actual ID of your range input
  ...
  .then(data => {
    ...
    function selectAnswer(selectedOption, correctAnswer) {
      ...
      options.forEach(optionButton => {
        ...
        // Highlight options that match the range value with yellow
        const rangeValue = parseInt(document.querySelector('.range').value);
        if (!isNaN(rangeValue) && parseInt(optionTextWithoutUnit) === rangeValue) {
          optionButton.style.background = 'linear-gradient(to right, #FFFF00, #FFD700)'; // Highlight matching option in yellow
        }
      });
